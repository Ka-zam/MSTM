...
contains
   ...
   subroutine output_header(iunit, inputfile)
      ...
      simulation_result%run_date_and_time = trim(rundate)//' '//trim(runtime)
      write (iunit, '(''****************************************************'')')
      write (iunit, '(''****************************************************'')')
      write (iunit, '('' mstm calculation results'')')
      write (iunit, '('' date, time:'')')
      write (iunit, '(a)') simulation_result%run_date_and_time
   ...
   subroutine print_run_variables(iunit)
      ...
      write (iunit, '(''****************************************************'')')
      write (iunit, '('' input variables for run '',i5)') simulation_result%run_number
      if (simulation_config%random_configuration) then
         ...
         if (target_shape .eq. 0) then
            ...
            write (iunit, '(3es12.4)') target_dimensions * simulation_config%length_scale_factor
         elseif (target_shape .eq. 1) then
            write (iunit, '('' cylindrical target, radius, half-thickness'')')
            write (iunit, '(3es12.4)') target_dimensions(1) * simulation_config%length_scale_factor, &
               target_dimensions(3) * simulation_config%length_scale_factor
         else
         ...
         if (number_components .gt. 1) then
            ...
            write (iunit, '(8es12.4)') simulation_config%component_ref_index(1:number_components)
         end if

         write (iunit, '('' sphere volume fraction'')')
         if ((.not. simulation_config%number_spheres_specified) .or. simulation_config%auto_target_radius) then
            write (iunit, '(es12.4)') simulation_config%sphere_volume_fraction
         else
         ...
         if (simulation_result%random_configuration_status .eq. 0) then
            write (iunit, '('' target configuration computed using random sampling + diffusion'')')
         elseif (simulation_result%random_configuration_status .eq. 1) then
            write (iunit, '('' target configuration computed using layered sampling + diffusion'')')
         elseif (simulation_result%random_configuration_status .eq. 2) then
      ...
      if (simulation_config%reflection_model) then
         if (simulation_config%auto_absorption_sample_radius) then
            write (iunit, '('' particle layer reflectance/absorptance model, absorption sample radius fraction'')')
            write (iunit, '(es12.4)') simulation_config%absorption_sample_radius_fraction
         else
            write (iunit, '('' particle layer reflectance/absorptance model, absorption sample radius'')')
            write (iunit, '(es12.4)') simulation_config%absorption_sample_radius * simulation_config%length_scale_factor
         end if
      end if
      ...
      if (number_plane_boundaries .gt. 0) then
         ...
         if (.not. simulation_config%incidence_average) then
            cb = cosd(simulation_config%incident_beta_degrees)
            call boundary_energy_transfer(simulation_result%incident_sin_beta, simulation_config%incident_direction, r, t, a)
      ...
      write (iunit, '(i10,3es12.4)') simulation_config%solver%max_iterations, simulation_config%solver%solution_epsilon, simulation_config%solver%mie_epsilon, interaction_radius
   ...
   subroutine print_calculation_results(fout)
      ...
      if (fout(1:7) .eq. 'console') then
         ...
         do
            ...
            if (trim(chartemp) .eq. simulation_result%run_date_and_time) exit
         end do
         do
            read (outunit, '(a)', iostat=io_status) chartemp
            if (io_status /= 0) then
            ...
            if (chartemp(1:28) .eq. ' calculation results for run') then
               read (outunit, *) i
               if (i .eq. simulation_result%run_number) exit
            end if
         end do
      end if
      if (simulation_config%configuration_average) then
         write (outunit, '('' averages collected: sample:'')')
         write (outunit, '(i5)') simulation_result%random_configuration_number * simulation_config%number_configuration_groups
      end if
      if (simulation_config%incidence_average) then
         write (outunit, '('' incidence averages collected: sample:'')')
         write (outunit, '(i5)') simulation_result%incident_direction_number * simulation_config%number_configuration_groups
      end if

      write (outunit, '('' number iterations, error, solution time '')')
      write (outunit, '(i6,3es12.4)') simulation_result%solution_iterations, simulation_result%solution_error, simulation_result%solution_time
      if (sphere_cluster%fft_translation_option .and. simulation_config%output%print_timings) then
      ...
      if (simulation_config%solver%solution_method(1:1) /= 'i') then
         write (outunit, '('' direct reciprocal condition estimate'')')
         write (outunit, '(es12.4)') simulation_result%reciprocal_condition
         if (simulation_config%output%print_timings) then
      ...
      if (simulation_config%output%print_sphere_data) then
         ...
         do n = 1, sphere_cluster%number_spheres
            if (simulation_config%configuration_average) then
               write (outunit, '(i8,4es12.4)') n, sphere_cluster%sphere_position(3, n), simulation_result%efficiency(1:2, 1, n), simulation_result%volume_absorption(1, n)
            else
               write (outunit, '(i8,3es12.4)') n, simulation_result%efficiency(1:2, 1, n), simulation_result%volume_absorption(1, n)
            end if
         end do
      end if

      if (periodic_lattice .or. simulation_config%reflection_model) then
         rl = simulation_result%plane_scattering(:, 2) - simulation_result%boundary_extinction(:, 0) + r(:)
!            tl=simulation_result%plane_scattering(:,1)+simulation_result%boundary_extinction(:,number_plane_boundaries+1)+t(:)
!            al=1.d0-rl-tl
         al = simulation_result%surface_absorptance(:)
         tl = 1.d0 - rl - al
...
!            write(outunit,'('' unit cell reflectance, absorptance, transmittance (unpol, par, perp)'')')
!               write(outunit,'(9es12.4)') 0.5d0*sum(simulation_result%plane_scattering(:,2)-simulation_result%boundary_extinction(:,0)+r(:)),simulation_result%total_efficiency(2,1), &
!                 .5d0*sum(simulation_result%plane_scattering(:,1)+simulation_result%boundary_extinction(:,number_plane_boundaries+1)+t(:)), &
!                 (simulation_result%plane_scattering(i,2)-simulation_result%boundary_extinction(i,0)+r(i),simulation_result%total_efficiency(2,i+1), &
!                  simulation_result%plane_scattering(i,1)+simulation_result%boundary_extinction(i,number_plane_boundaries+1)+t(i),i=1,2)

!            write(outunit,'('' down, up scattering fraction, total scattering cross section (unpol)'')')
!               write(outunit,'(9es12.4)') 0.5d0*sum(simulation_result%plane_scattering(:,2)),.5d0*sum(simulation_result%plane_scattering(:,1)), &
!                 pi*(0.5d0*sum(simulation_result%plane_scattering(:,2))+.5d0*sum(simulation_result%plane_scattering(:,1)))*sphere_cluster%cross_section_radius**2
         if (simulation_config%configuration_average .and. simulation_config%single_origin_expansion) then
            scacoef = (-0.5d0 * sum(simulation_result%diffuse_boundary_scattering(:, 0)) + 0.5d0 * sum(simulation_result%diffuse_boundary_scattering(:, 1))) &
                      * pi * sphere_cluster%cross_section_radius**2
            scarat = scacoef / (simulation_result%total_efficiency(3, 1) * pi * sphere_cluster%cross_section_radius**2)
            write (outunit, '('' down, up diffuse scattering fraction, optically thin dependent/independent ratio'')')
            write (outunit, '(9es12.4)') - 0.5d0 * sum(simulation_result%diffuse_boundary_scattering(:, 0)), &
               0.5d0 * sum(simulation_result%diffuse_boundary_scattering(:, number_plane_boundaries + 1)), &
               scarat
         ...
         if (simulation_result%efficiency_dimension .eq. 1) then
            write (outunit, '('' total extinction, absorption, scattering efficiencies (unpolarized incidence)'')')
            write (outunit, '(3es12.4)') simulation_result%total_efficiency(1:3, 1)
         else
            write (outunit, '('' total extinction, absorption, scattering efficiencies (unpol, par, perp incidence)'')')
            write (outunit, '(9es12.4)') simulation_result%total_efficiency(1:3, 1:3)
         end if
         if (.not. simulation_config%random_orientation) then
            if (number_plane_boundaries .gt. 0) then
               write (outunit, '(''  down and up extinction efficiencies (unpol, par, perp)'')')
               write (outunit, '(16es12.4)') &
              0.5d0 * sum(simulation_result%boundary_extinction(:, 0)), 0.5d0 * sum(-simulation_result%boundary_extinction(:, 1)), &
                  (simulation_result%boundary_extinction(i, 0), -simulation_result%boundary_extinction(i, 1), i=1, 2)
            end if
            if (simulation_config%calculate_up_down_scattering) then
               ...
               write (outunit, '(16es12.4)') &
              0.5d0 * sum(-simulation_result%boundary_scattering(:, 0)), 0.5d0 * sum(simulation_result%boundary_scattering(:, 1)), &
                  (-simulation_result%boundary_scattering(i, 0), simulation_result%boundary_scattering(i, 1), i=1, 2)
            end if
            if (number_plane_boundaries .gt. 0 .and. number_singular_points .gt. 0) then
               ...
               write (outunit, '(16es12.4)') &
                  0.5d0 * sum(simulation_result%evanescent_scattering(:)), (simulation_result%evanescent_scattering(i), i=1, 2)
            end if
         end if
      end if
      if (simulation_config%configuration_average .and. simulation_config%single_origin_expansion .and. .not. simulation_config%random_orientation) then
         ...
         write (outunit, '(6es12.4)') simulation_result%diffuse_cross_section_ratio, 2.d0 * imrieff, qeeff, &
            simulation_result%diffuse_cross_section_ratio * simulation_result%total_efficiency(3, 1) / (simulation_result%diffuse_cross_section_ratio * simulation_result%total_efficiency(3, 1) + simulation_result%total_efficiency(2, 1)), scarat
         write (outunit, '('' NI formulation ratio'')')
         write (outunit, '(5es12.4)') simulation_result%total_cross_section, simulation_result%diffuse_cross_section, &
            pi * (simulation_result%diffuse_cross_section) / (pi * sphere_cluster%cross_section_radius**2 * simulation_result%total_efficiency(1, 1))
      end if
      if (simulation_config%configuration_average .and. target_shape .eq. 2 .and. (.not. simulation_config%random_configuration_host)) then
         scarat = sphere_cluster%mean_qext_mie * sphere_cluster%area_mean_radius**2 * 3.d0 * dble(sphere_cluster%number_spheres) / (4.d0 * simulation_result%fit_radius**3)
         scarat = 2 * aimag(sphere_cluster%effective_ref_index) / scarat
         write (outunit, '('' mie fit effective ri, radius, RT ratio, status'')')
         write (outunit, '(4es12.4,i5)') sphere_cluster%effective_ref_index, simulation_result%fit_radius, scarat, simulation_result%fit_status
      end if
      if (simulation_config%configuration_average .and. target_shape .eq. 2 .and. simulation_config%calculate_near_field) then
         write (outunit, '('' field fit effective ri'')')
         write (outunit, '(2es12.4)') simulation_result%near_field_effective_ref_index
      end if

      if (simulation_config%calculate_scattering_matrix .and. .not. periodic_lattice) then
         if (simulation_config%output%normalize_s11) then
            s11scale = 1.d0 / ((sphere_cluster%cross_section_radius**2) * pi * simulation_result%total_efficiency(3, 1))
         else
!               s11scale=(sphere_cluster%cross_section_radius**2)*simulation_result%total_efficiency(3,1)
            s11scale = two_pi
!               if(.not.simulation_config%random_orientation) s11scale=pi*s11scale
         end if
         if (((.not. sphere_cluster%any_optically_active) .and. simulation_config%random_orientation) .or. simulation_config%azimuthal_average) then
            nsmat = 6
         ...
         if (simulation_config%random_orientation) then
            ...
            do i = simulation_result%scattering_matrix_lower_bound, simulation_result%scattering_matrix_upper_bound
               smt = scaled_scattering_matrix(simulation_result%scattering_matrix(1:16, i))
            ...
            if (simulation_config%scattering_map_model .eq. 0) then
               if (number_plane_boundaries .eq. 0) then
                  if (simulation_config%azimuthal_average) then
                     ...
                     if (simulation_config%incident_frame) then
                  ...
                  if ((simulation_config%configuration_average .or. simulation_config%incidence_average) .and. simulation_config%single_origin_expansion) then
!write(*,'(es12.4)') s11scale
                     write (outunit, '('' diffuse scattering matrix '')')
                     call print_scattering_matrix_header(no_numbers=.true.)
                     do i = simulation_result%scattering_matrix_lower_bound, simulation_result%scattering_matrix_upper_bound
                        smt = scaled_scattering_matrix(simulation_result%diffuse_scattering_matrix(1:16, i))
                        smt(1) = smt(1) * s11scale
                        call print_scattering_matrix_row(i, smt)
                     end do
                  end if
                  ...
                  call print_scattering_matrix_header(no_numbers=.true.)
                  do i = simulation_result%scattering_matrix_lower_bound, simulation_result%scattering_matrix_upper_bound
                     smt = scaled_scattering_matrix(simulation_result%scattering_matrix(17:32, i))
                     smt(1) = smt(1) * s11scale
                     call print_scattering_matrix_row(i, smt)
                  end do
               end if
               ...
               write (outunit, '(i10)') simulation_result%scattering_matrix_upper_bound
               ...
               do sy = -simulation_config%scattering_map_dimension, simulation_config%scattering_map_dimension
                  ...
                  do sx = -simulation_config%scattering_map_dimension, simulation_config%scattering_map_dimension
                     ...
                     s = s + 1
                     smt = scaled_scattering_matrix(simulation_result%scattering_matrix(1:16, s))
                     smt(1) = smt(1) * s11scale
               ...
               do sy = -simulation_config%scattering_map_dimension, simulation_config%scattering_map_dimension
                  ...
                  do sx = -simulation_config%scattering_map_dimension, simulation_config%scattering_map_dimension
                     ...
                     kx = dble(sx) / dble(simulation_config%scattering_map_dimension)
                     s = s + 1
                     smt = scaled_scattering_matrix(simulation_result%scattering_matrix(17:32, s))
                     smt(1) = smt(1) * s11scale
                     write (outunit, '(2f9.5)', advance='no') kx, ky
                     do j = 1, 16
                        write (outunit, '(es12.4)', advance='no') smt(smvec0(j))
         ...
         if (simulation_config%azimuthal_average .and. (.not. simulation_config%random_orientation) .and. (.not. simulation_config%numerical_azimuthal_average)) then
!               s11scale=1.d0/simulation_result%scattering_matrix_expansion(1,0,1)
!               simulation_result%scattering_matrix_expansion=simulation_result%scattering_matrix_expansion*s11scale
            ...
            do n = 0, 2 * sphere_cluster%t_matrix_order
               write (outunit, '(i5,6es14.6)') n, simulation_result%scattering_matrix_expansion(1, n, 1), simulation_result%scattering_matrix_expansion(16, n, 1), &
        0.5d0 * (simulation_result%scattering_matrix_expansion(2, n, 2) + simulation_result%scattering_matrix_expansion(5, n, 2)), &
      0.5d0 * (simulation_result%scattering_matrix_expansion(12, n, 2) + simulation_result%scattering_matrix_expansion(15, n, 2)), &
                  simulation_result%scattering_matrix_expansion(6, n, 3), simulation_result%scattering_matrix_expansion(6, n, 4)
            end do
            if (simulation_config%configuration_average) then
               write (outunit, '('' azimuthal averaged scattering matrix expansion coefficients, coherent field'')')
               write (outunit, '(''    n         11            44            12            34           22p           22m'')')
               do n = 0, 2 * sphere_cluster%t_matrix_order
                  write (outunit, '(i5,6es14.6)') n, simulation_result%coherent_scattering_expansion(1, n, 1), simulation_result%coherent_scattering_expansion(16, n, 1), &
    0.5d0 * (simulation_result%coherent_scattering_expansion(2, n, 2) + simulation_result%coherent_scattering_expansion(5, n, 2)), &
  0.5d0 * (simulation_result%coherent_scattering_expansion(12, n, 2) + simulation_result%coherent_scattering_expansion(15, n, 2)), &
                  simulation_result%coherent_scattering_expansion(6, n, 3), simulation_result%coherent_scattering_expansion(6, n, 4)
               end do
            end if
         end if
         if (simulation_config%random_orientation) then
...
!                &''a34           a13           a24           a14'')')
!               do n=0,2*sphere_cluster%t_matrix_order
!                  write(outunit,'(i5,11es14.6)') w,simulation_result%scattering_matrix_expansion(1,1,n),simulation_result%scattering_matrix_expansion(2,2,n),&
!                    simulation_result%scattering_matrix_expansion(3,3,n),simulation_result%scattering_matrix_expansion(2,3,n),simulation_result%scattering_matrix_expansion(3,2,n),simulation_result%scattering_matrix_expansion(4,4,n),&
!                    simulation_result%scattering_matrix_expansion(1,2,n),simulation_result%scattering_matrix_expansion(3,4,n),simulation_result%scattering_matrix_expansion(1,3,n),simulation_result%scattering_matrix_expansion(2,4,n),&
!                    simulation_result%scattering_matrix_expansion(1,4,n)
!               enddo
            write (outunit, '(''    n         11            44            12            34           22            33'')')
            do n = 0, 2 * sphere_cluster%t_matrix_order
               write (outunit, '(i5,11es14.6)') n, simulation_result%scattering_matrix_expansion(1, 1, n), simulation_result%scattering_matrix_expansion(4, 4, n), &
                  simulation_result%scattering_matrix_expansion(1, 2, n), simulation_result%scattering_matrix_expansion(3, 4, n), &
                  simulation_result%scattering_matrix_expansion(2, 2, n), simulation_result%scattering_matrix_expansion(3, 3, n)
            end do
            write (outunit, '('' orientation averaged coherent scattering matrix expansion coefficients'')')
            write (outunit, '(''    n         11            44            12            34           22            33'')')
            do n = 0, 2 * sphere_cluster%t_matrix_order
               write (outunit, '(i5,11es14.6)') n, simulation_result%coherent_scattering_expansion(1, 1, n), simulation_result%coherent_scattering_expansion(4, 4, n), &
               simulation_result%coherent_scattering_expansion(1, 2, n), simulation_result%coherent_scattering_expansion(3, 4, n), &
                  simulation_result%coherent_scattering_expansion(2, 2, n), simulation_result%coherent_scattering_expansion(3, 3, n)
            end do
         end if
      end if

      if (simulation_config%calculate_scattering_matrix .and. periodic_lattice) then
         ...
         write (outunit, '(2i6)') simulation_result%reflection_transmission_direction_counts(1), 16
         write (outunit, '(''    kx      ky '')', advance='no')
         do j = 1, 4
            do i = 1, 4
               write (outunit, '(''     '',2i1,''     '')', advance='no') i, j
         ...
         write (outunit, *)
         do i = 1, simulation_result%reflection_transmission_direction_counts(1)
            smt = scaled_scattering_matrix(simulation_result%scattering_matrix(1:16, i))
       write (outunit, '(2f9.5)', advance='no') simulation_result%reflection_transmission_vectors(1:2, i) / dble(layer_ref_index(0))
            do j = 1, 16
               write (outunit, '(es12.4)', advance='no') smt(smvec0(j))
            end do
            write (outunit, *)
         ...
         write (outunit, '('' forward hemisphere scattering'')')
         write (outunit, '('' number directions, number SM elements'')')
         write (outunit, '(2i6)') simulation_result%reflection_transmission_direction_counts(2), 16
         write (outunit, '(''    kx      ky '')', advance='no')
         do j = 1, 4
            do i = 1, 4
               write (outunit, '(''     '',2i1,''     '')', advance='no') i, j
         ...
         write (outunit, *)
         do i = 1, simulation_result%reflection_transmission_direction_counts(2)
            smt = scaled_scattering_matrix(simulation_result%scattering_matrix(17:32, i))
            write (outunit, '(2f9.5)', advance='no') &
               simulation_result%reflection_transmission_vectors(1:2, i) / dble(layer_ref_index(number_plane_boundaries))
            do j = 1, 16
               write (outunit, '(es12.4)', advance='no') smt(smvec0(j))
            end do
            write (outunit, *)
      ...
      if (simulation_config%random_orientation .and. simulation_config%configuration_average) then
         write (outunit, '('' mean t matrix elements (p=1,2)'')')
         do i = 1, sphere_cluster%t_matrix_order
            write (outunit, '(i5,4es12.4)') i, simulation_result%mean_t_matrix(1, i), simulation_result%mean_t_matrix(2, i)
         end do
      end if
   ...
   contains
      subroutine print_scattering_matrix_header(no_numbers)
         ...
         if (present(no_numbers)) then
            if (.not. no_numbers) then
               write (outunit, '('' number directions, number SM elements:'')')
               write (outunit, '(2i6)') simulation_result%scattering_matrix_upper_bound - simulation_result%scattering_matrix_lower_bound + 1, nsmat
            end if
         else
            write (outunit, '('' number directions, number SM elements:'')')
            write (outunit, '(2i6)') simulation_result%scattering_matrix_upper_bound - simulation_result%scattering_matrix_lower_bound + 1, nsmat
      ...
      subroutine print_scattering_matrix_row(i, smt)
         ...
         integer :: i
         real(real64) :: smt(16)
!            write(outunit,'(f8.2)',advance='no') dble(i)*180.d0/dble(simulation_result%scattering_matrix_upper_bound-simulation_result%scattering_matrix_lower_bound)
         write (outunit, '(f8.2)', advance='no') simulation_config%scattering_matrix_angle_minimum &
            + dble(i - simulation_result%scattering_matrix_lower_bound) / dble(simulation_result%scattering_matrix_upper_bound - simulation_result%scattering_matrix_lower_bound) * (simulation_config%scattering_matrix_angle_maximum - simulation_config%scattering_matrix_angle_minimum)
         do j = 1, nsmat
