...
contains
   ...
   subroutine general_interaction_matrix(matrix, mie_mult, mpi_comm)
      ...
      complex(real64), intent(out) :: matrix(sphere_cluster%number_eqns, sphere_cluster%number_eqns)
      ...
      do i = 1, sphere_cluster%number_spheres
         ...
         ilay = find_layer_index(sphere_cluster%sphere_position(3, i))
         do j = 1, sphere_cluster%number_spheres
            if (sphere_cluster%host_sphere(i) .ne. sphere_cluster%host_sphere(j) .and. sphere_cluster%host_sphere(j) .ne. i .and. sphere_cluster%host_sphere(i) .ne. j) cycle
            task = task + 1
            ...
            jlay = find_layer_index(sphere_cluster%sphere_position(3, j))
            allocate (acmat(2 * nbi, 2 * nbj))
            acmat = 0.0_real64
            if (sphere_cluster%host_sphere(i) .ne. 0 .or. sphere_cluster%host_sphere(j) .ne. 0) then
               if (sphere_cluster%host_sphere(j) .eq. i) then
                  rimedium = sphere_cluster%sphere_ref_index(:, i)
                  vtype = 1
                  i1 = sphere_cluster%sphere_offset(i) + sphere_cluster%sphere_block(i) + 1
                  i2 = sphere_cluster%sphere_offset(i) + 2 * sphere_cluster%sphere_block(i)
                  j1 = sphere_cluster%sphere_offset(j) + 1
                  j2 = sphere_cluster%sphere_offset(j) + sphere_cluster%sphere_block(j)
               elseif (sphere_cluster%host_sphere(i) .eq. j) then
                  rimedium = sphere_cluster%sphere_ref_index(:, j)
                  vtype = 1
                  j1 = sphere_cluster%sphere_offset(j) + sphere_cluster%sphere_block(j) + 1
                  j2 = sphere_cluster%sphere_offset(j) + 2 * sphere_cluster%sphere_block(j)
                  i1 = sphere_cluster%sphere_offset(i) + 1
                  i2 = sphere_cluster%sphere_offset(i) + sphere_cluster%sphere_block(i)
               elseif (sphere_cluster%host_sphere(i) .eq. sphere_cluster%host_sphere(j)) then
                  if (rdist .gt. interaction_radius) cycle
                  rimedium = sphere_cluster%sphere_ref_index(:, sphere_cluster%host_sphere(i))
                  i1 = sphere_cluster%sphere_offset(i) + 1
                  i2 = sphere_cluster%sphere_offset(i) + sphere_cluster%sphere_block(i)
                  j1 = sphere_cluster%sphere_offset(j) + 1
                  j2 = sphere_cluster%sphere_offset(j) + sphere_cluster%sphere_block(j)
           ...
           call generate_translation_matrix(sphere_cluster%sphere_order(j), sphere_cluster%sphere_order(i), translation_vector=rp, &
      ...
      if (numprocs .gt. 1) then
         nsend = sphere_cluster%number_eqns * sphere_cluster%number_eqns
         call parallel_reduce_sum(mpi_rank=0, &
                                  receive_buffer=matrix, mpi_number=nsend, mpi_comm=mpicomm)
      end if
      if (rank .eq. 0) then
         if (miemult) then
            allocate (anp(sphere_cluster%number_eqns))
            do j = 1, sphere_cluster%number_eqns
               if (any(abs(matrix(1:sphere_cluster%number_eqns, j)) .ne. 0.0_real64)) then
                  call apply_mie_coefficients(sphere_cluster%number_eqns, 1, 1, matrix(1:sphere_cluster%number_eqns, j), anp)
                  matrix(1:sphere_cluster%number_eqns, j) = -anp(1:sphere_cluster%number_eqns)
               end if
...
!
      if (firstrun) then
         if (smopt .and. sphere_cluster%store_translation_matrix) then
            ...
            do i = 1, nsphere - 1
               do j = i + 1, nsphere
                  if (sphere_cluster%host_sphere(j) .eq. sphere_cluster%host_sphere(i) &
                      .and. sphere_cluster%sphere_layer(j) .eq. sphere_cluster%sphere_layer(i)) then
                     rdist = sqrt(sum((sphere_cluster%sphere_position(:, i) - sphere_cluster%sphere_position(:, j))**2))
                     if (rdist .gt. interaction_radius) cycle
                     task = task + 1
         ...
         calcmat = (.not. (smopt .and. sphere_cluster%store_translation_matrix))
      end if
         ...
         do j = i + 1, nsphere
            if (sphere_cluster%host_sphere(j) .eq. sphere_cluster%host_sphere(i) &
               ...
               if (proc .eq. rank) then
                  ...
                  if (calcmat) then
                     nmin = min(noi, noj)
                     rot = (nmin .ge. sphere_cluster%translation_switch_order)
                     if (smopt .and. sphere_cluster%store_translation_matrix) then
             call stored_trans_mat(idim)%configure(3, sphere_cluster%sphere_position(:, i) - sphere_cluster%sphere_position(:, j), &
                                                              rimedium, rot)
                        loc_tranmat => stored_trans_mat(idim)
                     else
               call tranmat%configure(3, sphere_cluster%sphere_position(:, i) - sphere_cluster%sphere_position(:, j), rimedium, rot)
                        loc_tranmat => tranmat
                     end if
                  else
                     loc_tranmat => stored_trans_mat(idim)
                  ...
                  if (calcmat .and. (.not. (smopt .and. sphere_cluster%store_translation_matrix))) then
                     call tranmat%clear()
                  end if
               end if
            end if
...
!
   subroutine external_to_internal_expansion(neqns, nrhs, ain, bout, &
      ...
      if (present(geometry)) then
         ...
         call global_geometry%configure(sphere_cluster%host_sphere, sphere_cluster%sphere_block, sphere_cluster%sphere_offset, sphere_cluster%sphere_order, &
                                        sphere_cluster%sphere_position, sphere_cluster%sphere_ref_index)
